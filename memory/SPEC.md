...
## Pages
- `/` — create secret: textarea, up to 3 attachments (≤2 MB each), expiry select (1h/24h/7d), optional passphrase, burn-after-read checkbox → generates `/v/<id>#key=<b64>` link + QR dialog + private receipt link `/r/<token>`.
