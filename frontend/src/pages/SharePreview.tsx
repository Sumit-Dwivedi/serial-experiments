...
export default function SharePreview() {
  ...
  const title = meta('meta[property="og:title"]', "SERIAL://EXPERIMENTS");
  const description = meta('meta[property="og:description"]', "");
  const image = meta('meta[property="og:image"]', "/og-card.png");
  const site = meta('meta[property="og:site_name"]', "SERIAL://EXPERIMENTS");
  const host = window.location.host;
  const shown = url.split("#")[0];

  return (
    <PageShell>
      <p className="font-mono text-[11px] tracking-[0.4em] uppercase text-[#E8672E]">LAYER 07 // UNFURL INSPECTOR</p>
      <h1
      ...
      <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-[#6B6F76]">
        Chat apps fetch the page — never the fragment after <code className="text-[#E8672E]">#</code>.
      ...
      <label className="mt-8 block max-w-2xl">
        <span className="font-mono text-[11px] tracking-[0.2em] text-[#555961]">
          PASTE A SECRET LINK
        </span>
        <input
          ...
          className="mt-2 w-full border border-white/10 bg-[#0E0E10] px-3 py-2 font-mono text-xs text-[#D4CFC6] outline-none transition-colors duration-200 focus:border-[#E8672E]/50"
        />
      </label>

      <div className="mt-6 grid items-start gap-6 lg:grid-cols-2">
        {/* Slack */}
        <div className="border border-white/10 bg-[#17171A] p-5" data-testid="slack-preview">
          <p className="font-mono text-[11px] tracking-[0.22em] text-[#555961]">SLACK</p>
        ...
        {/* Discord */}
        <div className="border border-white/10 bg-[#17171A] p-5" data-testid="discord-preview">
          <p className="font-mono text-[11px] tracking-[0.22em] text-[#555961]">DISCORD</p>
          <div className="mt-4 bg-[#313338] p-4">
            <p className="text-[13px] break-all text-[#00A8FC]">{shown}</p>
            <div className="mt-2 max-w-md border-l-4 border-[#E8672E] bg-[#2B2D31] p-3">
      ...
      >
        <p className="text-sm text-[#B8C9B8]">
          ...
          <span className="font-mono text-xs text-[#B8C9B8]">
      ...
      <Link
        ...
        className="mt-8 inline-block font-mono text-[11px] text-[#555961] transition-colors duration-200 hover:text-[#E8672E]"
      >
        ‹ back to the vault
      </Link>
    </PageShell>
